    ...
    <AnimatePresence>
      {visible ? (
        <motion.div
          ...
          className="fixed inset-0 z-[100] flex flex-col items-center justify-center gap-8 bg-brand-yellow px-6"
        >
          {/* Floating deco squares, mirroring the tile geometry at a smaller scale. */}
          {[
            { className: "left-[10%] top-[15%] bg-brand-cyan", delay: "0s" },
            { className: "right-[15%] top-[20%] bg-brand-mint", delay: "0.5s" },
            { className: "bottom-[20%] left-[15%] bg-brand-pink", delay: "1s" },
          ].map((shape) => (
            <span
              key={shape.className}
              aria-hidden
              style={{ animationDelay: shape.delay }}
              className={`absolute h-12 w-12 animate-float-deco border-3 border-black sm:h-20 sm:w-20 ${shape.className}`}
            />
          ))}
          ...
          <div className="relative flex items-center gap-3 sm:gap-4">
            {TILES.map((tile) => (
              <motion.span
                ...
                className={`flex h-[90px] w-[90px] items-center justify-center border-4 border-black text-5xl font-bold text-black shadow-[8px_8px_0_#000] sm:h-[120px] sm:w-[120px] sm:border-6 sm:text-[6rem] sm:shadow-[12px_12px_0_#000] ${tile.className}`}
              >
          ...
          <p className="relative text-center text-xl font-bold text-black sm:text-2xl">
            {brand.name[locale]}
          ...
          <div className="relative h-4 w-[220px] overflow-hidden border-4 border-black bg-white shadow-[6px_6px_0_#000] sm:h-5 sm:w-[300px]">
            <motion.span
              initial={{ width: "0%" }}
              animate={{ width: "100%" }}
